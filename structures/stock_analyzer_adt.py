...
class StockAnalyzer:
    """Represent stock analyzer ADT, using static array"""
    def __init__(self, stocks, start=None, end=None, invest_num=None):
        """Initialize the analyzer"""
        self._stocks = Array(len(stocks))
        for i, stock in enumerate(stocks):
            self._stocks[i] = Stock(stock.name, stock.symbol, start, end)
    ...
    def process_stocks(self, tracker):
        """Get the most profitable stocks, using tracker"""
        stocks_rate = {}
        ...
        return self._profitable.slice(0, self._invest_num + 1)

    @staticmethod
    def get_combined_stock_dataframe(stocks, target_data):
        """Get combined stock dataframe from stocks and trecker"""
        stocks_frames = []
        for stock in stocks:
    ...
    def get_ratio_df(self, tracker):
        """Get ratio dataframe from stocks, based on a tracker"""
        if not self._profitable:
            self.process_stocks(tracker)
        combined_df = self.get_combined_stock_dataframe([item[0] for item
                                                         in self._profitable],
                                                        target_data=tracker)
        return combined_df.apply(lambda x: x / x[0])

    def get_portfolio_df(self, tracker):
        """Get the portfolio dataframe from stocks, based on a tracker"""
        if not self._profitable:
            self.process_stocks(tracker)
