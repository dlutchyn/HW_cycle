...
def plot():
    ...
    try:
        ...
        invest_num = None
    stocks = [Stock(elem[0], elem[1], start, end)
              for elem in users_stocks]
    names_stocks_portfolio = [item[0].name for item
                              in StockAnalyzer(stocks, start, end, invest_num).process_stocks('Adj Close')]
    return render_template('layout.html', img_source1=plot_portfolio(users_stocks, start, end, invest_num),
                           ...
                           len=len(names_stocks_portfolio), names_stocks_portfolio=names_stocks_portfolio)


@app.route("/check", methods=["POST"])
def check():
    for cb_stock_name in cb_stock_names:
        name = stocks_names[int(cb_stock_name[5:])][0]
        value = request.form.get(cb_stock_name)
        if value:
            users_stocks.append((name, value))
    for cb_for_name in cb_for_names:
        name = for_names[int(cb_for_name[6:])][0]
        value = request.form.get(cb_for_name)
        if value:
            users_stocks.append((name, value))
    for cb_cur_name in cb_cur_names:
        name = cur_names[int(cb_cur_name[6:])][0]
        value = request.form.get(cb_cur_name)
        if value:
            users_stocks.append((name, value))
    for cb_com_name in cb_com_names:
        name = com_names[int(cb_com_name[6:])][0]
